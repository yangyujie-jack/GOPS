...
class ApproxContainer(ApprBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.q: nn.Module = create_apprfunc(**value_args)

    def create_action_distributions(self, logits):
        return self.policy.get_act_dist(logits)


class PPOQ(AlgorithmBase):
    """PPO algorithm
    ...
    def local_update(self, data: DataDict, iteration: int) -> dict:
        ...
        with torch.no_grad():
            data["val"] = self.networks.q(data["obs"], data["act"])
    ...
    def _compute_loss(self, data: DataDict, iteration: int):
        ...
        # name completion
        mb_old_log_pro = data["logp"]
        mb_old_logits = data["logits"]
        ...
        mb_old_value = values
        mb_new_value = self.networks.q(data["obs"], data["act"])
